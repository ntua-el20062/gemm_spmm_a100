...
int main(int argc, char *argv[]) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(d_A, h_A, sizeof(data_type) * m*k, cudaMemcpyHostToDevice,stream));
    CUDA_CHECK(cudaMemcpyAsync(d_B, h_B, sizeof(data_type) * k*n, cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaEventRecord(h2d_stop, stream));

    // wait until the copies (and stop event) are finished
    ...
    CUDA_CHECK(cudaEventRecord(d2h_start, stream));

    CUDA_CHECK(cudaMemcpyAsync(h_C, d_C, sizeof(data_type) * sizeC, cudaMemcpyDeviceToHost,
                               stream));

    CUDA_CHECK(cudaEventRecord(d2h_stop, stream));
    // wait until the copies (and stop event) are finished
