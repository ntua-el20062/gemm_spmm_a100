...
log_path = "results_%_managed_C_mem_advise_10k_read_mostly.txt"
out_dir = "plots"
...
for (m, n, k), runs_same_size in sorted(by_size.items()):
    ...
    ax.set_title(f"GEMM iteration time (Managed, CPU writing C with mem advise & read mostly) | m={m}, n={n}, k={k}")
    ax.grid(alpha=0.3)
    ax.legend()
    ...
    out_png = os.path.join(out_dir, f"iters_managed_C_mem_advise_read_mostly.png")
    fig.tight_layout()
