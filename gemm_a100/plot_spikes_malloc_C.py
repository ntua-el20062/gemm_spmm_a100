...
log_path = "results_%_malloc_C_one_iter.txt"
out_dir = "plots"
spikes = [0, 250, 500, 750, 1000]


def split_runs(text: str):
    parts = re.split(r"\bMALLOC VERSION\b", text)
...
for (m, n, k), runs_same_size in sorted(by_size.items()):
    ...
    ax.set_title(f"GEMM iteration time (Malloc, CPU reading C) | m={m}, n={n}, k={k}")
    ax.grid(alpha=0.3)
    ax.legend()
    ...
    out_png = os.path.join(out_dir, f"iters_malloc_c_m{m}_n{n}_k{k}.png")
    fig.tight_layout()
